...
import com.allow.crazydots.p2p.ChatManager;
import com.allow.crazydots.p2p.ClientSocketHandler;
import com.allow.crazydots.p2p.GroupOwnerSocketHandler;
...
public class CrazyDotsRenderer implements GLSurfaceView.Renderer, Handler.Callback, CrazyDotsActivity.MessageTarget {
...
//    private Player currentPlayer;
    ...
    public CrazyDotsRenderer(final Context context, final int playMode, int rows, int cols, int numPlayers, WifiP2pInfo p2pInfo) {
        ...
        if (p2pInfo != null) {
            connectPlayers(p2pInfo);
        }

        playAgain(playMode, rows, cols, numPlayers);
    }

    private void connectPlayers(WifiP2pInfo p2pInfo) {
        Thread handler = null;
        if (p2pInfo.isGroupOwner) {
            Log.d(TAG, "Connected as group owner");
            try {
                handler = new GroupOwnerSocketHandler(
                        ((CrazyDotsActivity.MessageTarget) this).getHandler());
                handler.start();
            } catch (IOException e) {
                Log.d(TAG,
    ...
    public void pushMessage(String readMessage) {
        ...
        previousRow = Integer.parseInt(split[0]);
        previousCol = Integer.parseInt(split[1]);
        int row = Integer.parseInt(split[2]);
        int col = Integer.parseInt(split[3]);
        startingPlayerLine = Boolean.parseBoolean(split[4]);
        draggingPlayerLine = Boolean.parseBoolean(split[5]);
        draggingLineX1 = Float.parseFloat(split[6]);
        draggingLineY1 = Float.parseFloat(split[7]);
        draggingLineX2 = Float.parseFloat(split[8]);
        draggingLineY2 = Float.parseFloat(split[9]);
        move(row, col);
    }
    ...
    public boolean handleMessage(Message msg) {
        switch (msg.what) {
            case MESSAGE_READ:
                ...
                Log.d("MYTAG", "read: "+readMessage);
                Log.d(TAG, readMessage);
                pushMessage(readMessage);
                break;
...
//            draggingLineY2 = clamp(touchedPoint.y, bottomBound, topBound);
    ...
    private void makePlayerMove(int row, int col) {
        ...
        if (playMode == PLAY_MODE_MULTI && useP2p) {
            if (currentPlayer != chatManager.playerIdentity)
                return;
        }
